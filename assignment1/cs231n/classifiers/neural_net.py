...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #h1 = f(np.dot(X,W1) + b1) # calculate first hidden layer activations
    h1 = np.maximum(0, np.dot(X,W1) + b1) # ReLU activation function
    scores = np.dot(h1,W2) + b2 # output scores
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    grads['W2'] = np.dot(h1.T, dscores) # to get gradient of W2 we simply mul dscores to h1( h1 is considered the input X to the hidden layer)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      batch_indices = np.random.choice(num_train, batch_size) # pick batch_size random indices in range of (train samples size)
  ...
  def predict(self, X):
    ...
    ###########################################################################
    # predicted scores = X.W #y_pred row value is the class index with max score in each row in scores
